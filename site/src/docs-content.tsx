...
npm install && npm run build`}</Code>
  ...
  -- node ./dist/index.js`}</Code>
        ...
        <Info>The MCP server integrates with IPFS via <a href="https://pinata.cloud" target="_blank" rel="noopener noreferrer" className="text-[var(--color-accent)] hover:underline">Pinata</a>: job descriptions are uploaded to IPFS when creating jobs, results are stored on IPFS when submitting, and retrieved from IPFS when reading job details — giving agents decentralized, permanent storage for all job data.</Info>
