...
const docs: { slug: string; title: string; content: string }[] = [
  ...
  { slug: 'ows', title: 'Open Wallet Standard', content: 'OWS (Open Wallet Standard) by MoonPay — secure key management for AI agents. Partners: TON Foundation, PayPal, OKX, Solana Foundation. ENACT is the first TON project to integrate OWS. Private keys never leave the vault. Install: npm install -g @open-wallet-standard/core or curl installer. Create wallet: ows wallet create --name agent-treasury. Use signer callback instead of raw secretKey: contract.sendTransfer({ signer: owsSigner.sign }). OWS uses BIP-39 + SLIP-10 derivation at m/44\'/607\'/0\' — same mnemonic produces DIFFERENT addresses vs Tonkeeper (by design, OWS is multi-chain). Policy engine enforces rules before signing: value limits, rate limits. OWS is a local CLI/SDK, no built-in MCP server. Use programmatically alongside ENACT MCP. Source: examples/ows-integration/. GitHub: github.com/open-wallet-standard/core.' },
...
OWS (Open Wallet Standard): ENACT is the FIRST TON project to integrate OWS by MoonPay. OWS stores private keys in encrypted vault (AES-256-GCM), AI agents sign transactions without touching keys. Use signer callback: contract.sendTransfer({ signer: owsSigner.sign }) instead of secretKey. Partners: TON Foundation, PayPal, OKX, Solana Foundation. OWS uses BIP-39 + SLIP-10 derivation — same mnemonic gives DIFFERENT addresses vs Tonkeeper (by design). Install: npm i -g @open-wallet-standard/core or curl -fsSL https://docs.openwallet.sh/install.sh | bash. Policy engine enforces rules before signing. OWS is a local CLI/SDK — no built-in MCP server. Use OWS programmatically in agent code alongside ENACT MCP. Docs: /docs/ows. Source: examples/ows-integration/. GitHub: github.com/open-wallet-standard/core.
