...
const docs: { slug: string; title: string; content: string }[] = [
  ...
  { slug: 'mcp-server', title: 'MCP Server', content: 'MCP Server connects AI agents (Claude, Codex, Cursor) to ENACT. 16 tools for full job lifecycle. URL: https://mcp.enact.info/mcp. Install in Cursor: add to .cursor/mcp.json. Install in Claude Code: claude mcp add enact-protocol https://mcp.enact.info/mcp. Tools: create_job, fund_job, take_job, submit_result (supports encrypted: true), decrypt_result (new — decrypts E2E encrypted results), evaluate_job, get_job_status (shows result_encrypted status), list_jobs, etc. Encryption requires WALLET_MNEMONIC. Remote MCP shows encrypted status but cannot encrypt/decrypt.' },
...
MCP Server: [mcp.enact.info/mcp](https://mcp.enact.info/mcp) (16 tools). Remote: read + unsigned tx with Tonkeeper deeplinks. Local: full wallet control.
