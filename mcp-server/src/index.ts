...
server.tool(
    ...
    async ({ evaluator_address, budget_ton, description, file_path, timeout_seconds, evaluation_timeout_seconds }) => {
        ...
        const result = await sendTransaction(config.factoryAddress, toNano('0.008'), body);
...
server.tool(
    ...
    async ({ job_address, amount_ton }) => {
        ...
        const total = toNano(amount_ton) + toNano('0.006');
...
server.tool(
    ...
    async ({ job_address }) => {
        ...
        const result = await sendTransaction(Address.parse(job_address), toNano('0.006'), body);
        return { content: [{ type: 'text' as const, text: JSON.stringify(result) }] };
    }
);
...
server.tool(
    ...
    async ({ job_address, approved, reason }) => {
        ...
        const result = await sendTransaction(Address.parse(job_address), toNano('0.025'), body);
        return { content: [{ type: 'text' as const, text: JSON.stringify(result) }] };
    }
);
...
server.tool(
    ...
    async ({ evaluator_address, budget_usdt, description, timeout_seconds, evaluation_timeout_seconds }) => {
        ...
        const result = await sendTransaction(config.jettonFactoryAddress, toNano('0.008'), body);
        ...
        if (wallet) {
            try {
                ...
                await sendTransaction(jobAddr, toNano('0.006'), setBody);
            } catch (e: any) {
...
server.tool(
    ...
    async ({ job_address }) => {
        ...
        const body = beginCell()
            ...
            .endCell();
        const result = await sendTransaction(jobAddr, toNano('0.006'), body);
...
server.tool(
    ...
    async ({ job_address, amount_usdt }) => {
        ...
        const result = await sendTransaction(senderJettonWallet, toNano('0.025'), jettonBody);
