...
bot.callbackQuery('check_created', async (ctx) => {
    ...
    try {
        ...
        const fundAmount = toNano(pending.budgetTon) + toNano('0.006');
...
bot.command('create', async (ctx) => {
    ...
    try {
        ...
        if (mode === 'tonconnect') {
            ...
            const createLink = tonTransferLink(FACTORY_ADDRESS, toNano('0.008'), createBody);
            const fundLink = tonTransferLink(predictedJobAddr.toString(), toNano(budgetTon) + toNano('0.006'), fundBody);
        ...
        await sendTx(client, w, Address.parse(FACTORY_ADDRESS), toNano('0.008'), createBody);
        ...
        try {
            const fundBody = beginCell().storeUint(JobOpcodes.fund, 32).endCell();
            await sendTx(client, w, jobAddr, toNano(budgetTon) + toNano('0.006'), fundBody);
...
bot.command('createjetton', async (ctx) => {
    ...
    try {
        ...
        if (mode === 'tonconnect') {
            ...
            const createLink = tonTransferLink(JETTON_FACTORY_ADDRESS, toNano('0.008'), createBody);
            ...
            const setWalletLink = tonTransferLink(predictedAddr.toString(), toNano('0.006'), setWalletBody);
            ...
            const fundBody = beginCell()
                ...
                .storeBit(false).storeCoins(toNano('0.005')).storeBit(false)
                .endCell();
            const fundLink = tonTransferLink(clientJw.toString(), toNano('0.025'), fundBody);
        ...
        await sendTx(client, w, Address.parse(JETTON_FACTORY_ADDRESS), toNano('0.008'), createBody);
        ...
        await sendTx(client, w, jobAddr, toNano('0.006'), setJwBody);
...
bot.command('submit', async (ctx) => {
    ...
    try {
        ...
        if (mode === 'tonconnect') {
            const link = tonTransferLink(jobAddr.toString(), toNano('0.006'), body);
            const kb = new InlineKeyboard()
        ...
        await sendTx(client, w, jobAddr, toNano('0.006'), body);
    ...
    try {
        ...
        if (isJetton) {
            ...
            if (currentJw !== expectedJw.toString()) {
                ...
                const setLink = tonTransferLink(jobAddr.toString(), toNano('0.006'), setBody);
                const kb = new InlineKeyboard()
        ...
        if (isJetton && mode === 'tonconnect') {
            ...
            const fundBody = beginCell()
                ...
                .storeBit(false).storeCoins(toNano('0.005')).storeBit(false)
                .endCell();
            const link = tonTransferLink(clientJw.toString(), toNano('0.025'), fundBody);
            const kb = new InlineKeyboard()
        ...
        if (!isJetton && mode === 'tonconnect') {
            ...
            const amount = status.budget + toNano('0.006');
        ...
        if (isJetton) {
            ...
            await sendTx(client, w, clientJw, toNano('0.025'), jettonBody);
        } else {
            const body = beginCell().storeUint(JobOpcodes.fund, 32).endCell();
            const amount = status.budget + toNano('0.006');
...
bot.on(['message:photo', 'message:document'], async (ctx, next) => {
    ...
    try {
        ...
        const createGas = isJetton ? toNano('0.008') : toNano('0.008');
